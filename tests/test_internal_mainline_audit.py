...
def test_internal_mainline_audit_cli_source_set_is_clean():
    ...
    assert any(str(path).endswith("data/superpoint_query_extractor.py") for path in paths)
    assert any(str(path).endswith("gaussian/map.py") for path in paths)
    assert any(str(path).endswith("gaussian/renderer.py") for path in paths)
    assert any(str(path).endswith("gaussian/visibility.py") for path in paths)
    ...
    assert any(str(path).endswith("sparse/candidate_completion_runner.py") for path in paths)
    assert any(str(path).endswith("sparse/candidate_coverage.py") for path in paths)
    assert any(str(path).endswith("sparse/candidate_shard_builder.py") for path in paths)
    ...
    assert any(str(path).endswith("simulation/render_runner.py") for path in paths)
    assert any(str(path).endswith("audit_internal_pose_map_frame.py") for path in paths)
    assert any(str(path).endswith("extract_internal_superpoint_features_for_queries.py") for path in paths)
    assert any(str(path).endswith("teacher/distillation_artifact.py") for path in paths)
    assert any(str(path).endswith("teacher/geometric_observations.py") for path in paths)
    ...
    assert any(str(path).endswith("build_internal_query_feature_cache.py") for path in paths)
    assert any(str(path).endswith("build_internal_query_feature_cache_from_images.py") for path in paths)
    assert any(str(path).endswith("build_internal_query_feature_cache_from_maps.py") for path in paths)
    assert any(str(path).endswith("build_internal_query_feature_cache_from_superpoint_dir.py") for path in paths)
    assert any(str(path).endswith("build_internal_render_manifest.py") for path in paths)
    assert any(str(path).endswith("build_internal_render_teacher_jobs.py") for path in paths)
    ...
    assert any(str(path).endswith("run_internal_mainline_smoke.py") for path in paths)
    assert any(str(path).endswith("render_internal_3dgs_assets.py") for path in paths)
    ...
    assert any(str(path).endswith("build_internal_teacher_observations_from_geometry.py") for path in paths)
