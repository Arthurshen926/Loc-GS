...
def load_superpoint_model(*, weights: str | Path, device: torch.device) -> torch.nn.Module:
...
def build_manifest(
    *,
    ...
    split = reject_test_split(split_name, purpose="internal SuperPoint query feature extraction manifest")
    return {
        "schema_version": "internal_superpoint_query_feature_extraction_manifest_v1",
        ...
        "inference_stage": "query_superpoint_feature_extraction",
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Extract Loc-GS internal SuperPoint features for a selected query-id list."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    split = reject_test_split(str(args.split_name), purpose="internal SuperPoint query feature extraction")
    ...
    command = [
        sys.executable,
        "-m",
        "loc_gs.scripts.extract_internal_superpoint_features_for_queries",
        *(argv or sys.argv[1:]),
    ]
