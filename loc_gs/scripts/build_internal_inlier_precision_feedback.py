...
def build_manifest(
    *,
    ...
    split = reject_test_split(split_name, purpose="internal inlier precision feedback manifest")
    return {
        "schema_version": "internal_inlier_precision_feedback_manifest_v1",
        ...
        "inference_stage": "inlier_precision_feedback",
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build inlier precision feedback from internal cached sparse eval outputs.")
    ...
    parser.add_argument("--selected_gap_gain", type=float, default=1.0)
    parser.add_argument("--inlier_gap_gain", type=float, default=2.0)
    parser.add_argument("--post_pnp_harm_gain", type=float, default=1.0)
    parser.add_argument("--max_distill_weight", type=float, default=4.0)
    return parser


def main(argv: list[str] | None = None) -> int:
    args = build_argparser().parse_args(argv)
    split = reject_test_split(str(args.split_name), purpose="internal inlier precision feedback")
    ...
    cfg = InlierPrecisionFeedbackConfig(
        ...
        selected_gap_gain=float(args.selected_gap_gain),
        inlier_gap_gain=float(args.inlier_gap_gain),
        post_pnp_harm_gain=float(args.post_pnp_harm_gain),
        max_distill_weight=float(args.max_distill_weight),
    )
    feedback = build_inlier_precision_feedback(rows, metrics=metrics, scene=str(args.scene), split_name=split, cfg=cfg)
    command = [sys.executable, "-m", "loc_gs.scripts.build_internal_inlier_precision_feedback", *(argv or sys.argv[1:])]
    hyperparameters = {
        "selected_correct_ratio_floor": float(args.selected_correct_ratio_floor),
        "inlier_correct_ratio_floor": float(args.inlier_correct_ratio_floor),
        "target_median_te_cm": float(args.target_median_te_cm),
    ...
    (args.output_dir / "inlier_precision_feedback.json").write_text(
        json.dumps(feedback, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    (args.output_dir / "inlier_precision_feedback.jsonl").write_text(
        "\n".join(json.dumps(row, sort_keys=True) for row in feedback["per_query"]) + "\n",
        encoding="utf-8",
    )
