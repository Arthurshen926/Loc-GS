...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build sparse-pixel source rays from projected Gaussian proxy contributors."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    matches_manifest, matches = _load_records(Path(args.matches), record_key="match", type_names={"match", "record"})
    projections_manifest, projections = _load_records(
        Path(args.projections),
        record_key="projection",
        type_names={"projection", "record"},
    )
    matches_split = validate_split_name(_manifest_split(matches_manifest, split_name))
    projections_split = validate_split_name(_manifest_split(projections_manifest, split_name))
    ...
    manifest = {
        ...
        "projections": str(Path(args.projections)),
        ...
        "source_manifests": {
            ...
            "projections": projections_manifest,
        },
    }
    ...
    split_audit = {
        ...
        "projections_split_name": projections_split,
        "test_split_used": False,
        "notes": "Projected Gaussian ray builder rejects test split inputs.",
    }
