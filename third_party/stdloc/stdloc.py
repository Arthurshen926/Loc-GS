...
def _sample_depth(depth_map, points2d, interpolation="nearest"):
    if points2d.numel() == 0:
        return depth_map.new_zeros((0,))

    height, width = depth_map.shape[-2:]
    x = points2d[:, 0].clamp(0, width - 1)
    y = points2d[:, 1].clamp(0, height - 1)

    if interpolation == "nearest":
        x_idx = x.long().clamp(0, width - 1)
        y_idx = y.long().clamp(0, height - 1)
        return depth_map[y_idx, x_idx]

    if interpolation != "bilinear":
        raise ValueError(f"Unsupported depth interpolation: {interpolation}")

    x0 = torch.floor(x).long()
    y0 = torch.floor(y).long()
    x1 = (x0 + 1).clamp(max=width - 1)
    y1 = (y0 + 1).clamp(max=height - 1)
    wx = (x - x0.float()).to(depth_map.dtype)
    wy = (y - y0.float()).to(depth_map.dtype)
...
def lift_2d_to_3d(points2d, intrinsic, Twc, depth_map, interpolation="nearest"):
    """
    points2d: tensor [N, 2]
    ...
    depth_values = _sample_depth(depth_map, points2d, interpolation=interpolation)
...
def soft_argmax_offsets(scores, window_size, temperature=0.1):
    """
    scores: tensor [N, window_size * window_size]
    Returns x/y offsets inside each local window.
    """
    if scores.numel() == 0:
    ...
    y, x = torch.meshgrid(
        torch.arange(window_size, device=scores.device, dtype=scores.dtype),
        torch.arange(window_size, device=scores.device, dtype=scores.dtype),
        indexing="ij",
    )
...
def sample_gaussians(gaussians: GaussianModel, idx_sampled):
    ...
    if hasattr(gaussians, "_locability_logit"):
        sampled_gaussians._locability_logit = gaussians._locability_logit[idx_sampled]
...
def apply_landmark_prior(corr_matrix, landmark_prior=None, weight=0.0):
    if landmark_prior is None or weight <= 0:
        return corr_matrix

    prior = landmark_prior.to(device=corr_matrix.device, dtype=corr_matrix.dtype)
    prior = prior.reshape(-1)
    if prior.shape[0] != corr_matrix.shape[-1]:
        return corr_matrix

    prior = prior.clamp(0.0, 1.0)
    centered_prior = prior - prior.mean()
    return corr_matrix + float(weight) * centered_prior.reshape(1, -1)


def apply_rendered_prior(corr_matrix, rendered_prior=None, weight=0.0):
    if rendered_prior is None or weight <= 0:
        return corr_matrix

    prior = rendered_prior.to(device=corr_matrix.device, dtype=corr_matrix.dtype)
    prior = prior.reshape(-1)
    if prior.shape[0] != corr_matrix.shape[-1]:
        return corr_matrix
    ...
    prior = prior.clamp(0.0, 1.0)
    centered_prior = prior - prior.mean()
    prior_shape = [1] * (corr_matrix.dim() - 1) + [-1]
    return corr_matrix + float(weight) * centered_prior.reshape(prior_shape)
...
def select_eval_cameras(cameras, max_cameras=None, stride=1):
    stride = max(1, int(stride or 1))
    if hasattr(cameras, "__getitem__"):
        selected = cameras[::stride]
    ...
    for idx, camera in enumerate(cameras):
        ...
        if max_cameras is not None and max_cameras > 0 and len(selected) >= max_cameras:
            break
    return selected
...
class STDLoc:
    def __init__(self, gaussians, config):
        ...
        self.landmark_prior = self._load_landmark_prior(sampled_idx)
    ...
    def _load_landmark_prior(self, sampled_idx):
        sampled_idx_cpu = torch.as_tensor(sampled_idx).detach().cpu().long()
        score_path = self.config["sparse"].get(
            "landmark_score_path", "detector/sampled_scores.pkl"
        )
        full_path = os.path.join(self.config["model_path"], score_path)
        if not os.path.exists(full_path):
            return None

        scores = pickle.load(open(full_path, "rb"))
        if isinstance(scores, dict):
            if "sampled_scores" in scores:
                scores = scores["sampled_scores"]
            elif "score_avg" in scores:
                scores = scores["score_avg"][sampled_idx_cpu]
        scores = torch.as_tensor(scores, dtype=torch.float32, device="cuda").reshape(-1)
        if scores.shape[0] != sampled_idx_cpu.shape[0]:
            return None
    ...
    def loc_sparse(self, query_feature_map, fovx, fovy):
        ...
        corr_matrix = apply_landmark_prior(
            corr_matrix,
            self.landmark_prior,
            weight=self.config["sparse"].get("landmark_prior_weight", 0.0),
        )
    ...
    def loc_dense(
        ...
        rendered_locability_map = render_pkg.get("locability_map")
        ...
        coarse_rendered_locability = None
        if rendered_locability_map is not None:
            coarse_rendered_locability = F.interpolate(
                rendered_locability_map[None],
                size=(Hc, Wc),
                mode="bilinear",
                align_corners=False,
            )[0]
        ...
        locability_prior_weight = self.config["dense"].get("locability_prior_weight", 0.0)
        coarse_corr_matrix = apply_rendered_prior(
            coarse_corr_matrix,
            coarse_rendered_locability,
            weight=locability_prior_weight,
        )
        ...
        if rendered_locability_map is not None and locability_prior_weight > 0:
            rendered_locability_windows = (
                F.unfold(
                    rendered_locability_map[None],
                    (W, W),
                    stride=W,
                ...
                .reshape(1, 1, WW, -1)[c_b_ids, :, :, c_j_ids]
                .squeeze(1)
            )
            rendered_locability_windows = rendered_locability_windows.clamp(0.0, 1.0)
            rendered_locability_windows = (
                rendered_locability_windows
                - rendered_locability_windows.mean(dim=-1, keepdim=True)
            )
            fine_scores = (
                fine_scores
                + float(locability_prior_weight)
                * rendered_locability_windows[:, None, :]
            )

        fine_corr_matrix = dual_softmax(
            fine_scores, temp=self.config["dense"]["fine_dual_softmax_temp"]
        )
        ...
        if self.config["dense"].get("subpixel_refine", False):
            subpixel_temp = self.config["dense"].get("subpixel_temperature", 0.1)
            query_offsets = soft_argmax_offsets(
                fine_scores[f_b_ids, :, f_j_ids], W, temperature=subpixel_temp
            )
            rendered_offsets = soft_argmax_offsets(
                fine_scores[f_b_ids, f_i_ids, :], W, temperature=subpixel_temp
            )
        else:
            query_offsets = torch.stack([f_i_ids % W, f_i_ids // W], dim=1).float()
            rendered_offsets = torch.stack([f_j_ids % W, f_j_ids // W], dim=1).float()

        query_window_origins = torch.stack(
            [c_i_ids[f_b_ids] % Wc * W, c_i_ids[f_b_ids] // Wc * W],
            dim=1,
        ).float()
        rendered_window_origins = torch.stack(
            [c_j_ids[f_b_ids] % Wc * W, c_j_ids[f_b_ids] // Wc * W],
            dim=1,
        ).float()
        query_p2d = query_window_origins + query_offsets
        rendered_p2d = rendered_window_origins + rendered_offsets
        ...
        p3d = lift_2d_to_3d(
            ...
            depth,
            interpolation=self.config["dense"].get("depth_interpolation", "nearest"),
        )
