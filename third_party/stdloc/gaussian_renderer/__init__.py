...
def get_visible_locability_values(pc: GaussianModel, visible_mask):
    try:
    ...
    locability = locability[visible_mask]
    if locability.dim() == 1:
        locability = locability[:, None]
    return locability.clamp(0.0, 1.0)
...
def get_render_visible_mask_2dgs(
    ...
    if rasterization_2dgs is None:
        raise ImportError("The active gsplat package does not provide rasterization_2dgs")
...
def render_gsplat(
    ...
    **rasterize_args
    ...
    if rgb_only is False:
        ...
        locability_values = get_visible_locability_values(pc, visible_mask)
...
def render_gsplat_2dgs(
    ...
    **rasterize_args
    ...
    if rgb_only is False:
        ...
        locability_values = get_visible_locability_values(pc, visible_mask)
        if locability_values is not None and locability_values.numel() > 0:
            locability_map, _, _, _, _, _, _ = rasterization_2dgs(
                means=means3D[visible_mask],
                quats=rotations[visible_mask],
                scales=scales[visible_mask],
                opacities=opacity.squeeze(-1)[visible_mask],
