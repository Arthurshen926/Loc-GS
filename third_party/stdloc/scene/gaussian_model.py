...
class GaussianModel_2dgs(nn.Module):
    ...
    def restore(self, model_args, training_args):
        ...
        try:
            ...
            print(
                "[Warning] Optimizer state is incompatible with the current "
                f"Gaussian parameters; using a fresh optimizer. Details: {exc}"
            )
    ...
    def save_ply(self, path):
        ...
        for idx, attribute in enumerate(elements.dtype.names):
            elements[attribute] = attributes[:, idx]
...
class GaussianModel(nn.Module):
    ...
    def __init__(self, sh_degree : int):
        ...
        self.setup_functions()
        self._loc_feature = torch.empty(0)
        self._locability_logit = torch.empty(0)
    ...
    def restore(self, model_args, training_args):
        if len(model_args) == 14:
            ...
            self.spatial_lr_scale,
            self._loc_feature,
            self._locability_logit) = model_args
        else:
            (self.active_sh_degree,
            self._xyz,
            self._features_dc,
            ...
            self.spatial_lr_scale,
            self._loc_feature) = model_args
            self._locability_logit = torch.zeros_like(self._opacity)
        self.training_setup(training_args)
        self.xyz_gradient_accum = xyz_gradient_accum
        self.denom = denom
        try:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, loc_feature_size : int, speedup: bool):
        ...
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        self._loc_feature = nn.Parameter(_loc_feature.transpose(1, 2).contiguous().requires_grad_(True))
        self._locability_logit = nn.Parameter(torch.zeros((self.get_xyz.shape[0], 1), device="cuda").requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"},
            {'params': [self._loc_feature], 'lr':training_args.loc_feature_lr, "name": "loc_feature"},
            {'params': [self._locability_logit], 'lr':getattr(training_args, "locability_lr", training_args.loc_feature_lr * 0.1), "name": "locability"},
        ]
    ...
    def save_ply(self, path):
        ...
        scale = self._scaling.detach().cpu().numpy()
        rotation = self._rotation.detach().cpu().numpy()
        locability_logit = self._locability_logit.detach().cpu().numpy()
        loc_feature = self._loc_feature.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()

        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes()]

        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, locability_logit, loc_feature), axis=1)
    ...
    def load_ply(self, path):
        ...
        loc_feature = np.stack([np.asarray(plydata.elements[0][f"loc_{i}"]) for i in range(count)], axis=1)
        loc_feature = np.expand_dims(loc_feature, axis=-1)
        if "locability_logit" in plydata.elements[0].data.dtype.names:
            locability_logit = np.asarray(plydata.elements[0]["locability_logit"])[..., np.newaxis]
        else:
            locability_logit = np.zeros_like(opacities)
        ...
        self._rotation = nn.Parameter(torch.tensor(rots, dtype=torch.float, device="cuda").requires_grad_(True))
        self._loc_feature = nn.Parameter(torch.tensor(loc_feature, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
        self._locability_logit = nn.Parameter(torch.tensor(locability_logit, dtype=torch.float, device="cuda").requires_grad_(True))
        self.active_sh_degree = self.max_sh_degree
    ...
    def prune_points(self, mask):
        ...
        self._rotation = optimizable_tensors["rotation"]
        self._loc_feature = optimizable_tensors["loc_feature"]
        self._locability_logit = optimizable_tensors["locability"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_loc_feature, new_locability_logit):
        d = {"xyz": new_xyz,
        "f_dc": new_features_dc,
        "f_rest": new_features_rest,
        "opacity": new_opacities,
        ...
        "rotation" : new_rotation,
        "loc_feature": new_loc_feature,
        "locability": new_locability_logit}
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_opacity = self._opacity[selected_pts_mask].repeat(N,1)
        new_loc_feature = self._loc_feature[selected_pts_mask].repeat(N,1,1)
        new_locability_logit = self._locability_logit[selected_pts_mask].repeat(N,1)

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_loc_feature, new_locability_logit)
        prune_filter = torch.cat((selected_pts_mask, torch.zeros(N * selected_pts_mask.sum(), device="cuda", dtype=bool)))
        self.prune_points(prune_filter)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_rotation = self._rotation[selected_pts_mask]
        new_loc_feature = self._loc_feature[selected_pts_mask]
        new_locability_logit = self._locability_logit[selected_pts_mask]

        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_loc_feature, new_locability_logit)
